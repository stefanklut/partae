...
cp -r -T $SEPARATION/ $tmp_dir/separation
...
if [[ -f $SEPARATION/.git ]]; then
	...
	rm separation/.git && cp -r $GIT_DIR separation/.git
	sed 's/.*worktree.*/\tworktree = ../' separation/.git/config > separation/.git/config
fi
...
docker build --no-cache . -t docker.separation

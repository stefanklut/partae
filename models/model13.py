# Cosine similarity between scans from the same document should be high, add extra conv to image encoder and larger image size, predict start, end and middle of the document, BCE loss, no inverted shape, encode the shape info before passing it to LSTM
